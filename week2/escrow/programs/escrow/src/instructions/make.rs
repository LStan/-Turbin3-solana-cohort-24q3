...
pub struct Make<'info> {
    ...
    #[account(
        ...
        seeds = [b"escrow", maker.key().as_ref(), seed.to_le_bytes().as_ref(), mint_a.key().as_ref(), mint_b.key().as_ref()],
        bump
    )]
...
impl<'info> Make<'info> {
    pub fn create_escrow(&mut self, seed: u64, receive: u64, bump: u8) -> Result<()> {
